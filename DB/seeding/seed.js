...
let createWriteRecord = () => {
  ...
  for (var i = 0; i < numListings; i++) {
   ...
   let record = createRecord(i);
    // format data
    let listingRecord = `${record.listing.listingId}, ${record.listing.occupency}, \
${record.listing.feeNightly}, ${record.listing.feeService}, ${record.listing.feeCleaning}, \
${record.listing.rating}, ${record.listing.numRatings}, `;

    let reservationRecord = '';
    for (var j = 0; j < record.reservations.length; j++) {
      reservationRecord += `${resId}, ${record.reservations[j].listingKey}, ${record.reservations[j].start}, \
